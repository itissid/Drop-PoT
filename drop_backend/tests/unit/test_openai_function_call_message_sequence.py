...
2. R: Messages are being replayed(like if the last communication was in error) or we communicate with in loop human 
Difference is in serialization and sending these messages to the AI.

Notation:
S = Sytem message
U_i = ith(User) message with or without function calling requested.
U_i' = ith(User) message for replay, never sent with the function call argument that was/wasn't originally in U_i
A_j = jth(AI) message
A_j' = jth(AI) message for replay, sent to the AI with "function_call": {"name": ..., "arguments":...} format and null "content" key 
...
G/R:                                     S -> U_i
Messages for OpenAI:                     S -> U_i # Generated by the EventNode.context_to_openai_api_messages 
...
X:                                     S -> U_i -> A_i+1            # Original sequence from disk is S -> U_i -> A_i+1
...
R:                                     S -> U_i -> A_i+1 -> U_i+2
Messages for OpenAI:                   S -> U_i -> A_i+1 -> U_i+2 # No fn call Generated by the EventNode.context_to_openai_api_messages 
...
Messages for OpenAI:                   S -> U_i' -> A_i+1' -> F_i+2*  # Generated by the EventNode.context_to_openai_api_messages 
...
(TBD: What is saved on the disk? Should it be the actual messages we generated. Currently the messages saved to disk are used to )
"""
