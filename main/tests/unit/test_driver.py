...
class TestAIDriver(unittest.TestCase):
    ...
    def create_driver_wrapper_instance(self, MockedAltAI, events, system_message, mock_event_formatter_fn, mock_function_call_spec_callable, interrogation_callback=None):
    ...
    def assert_event_node(self, event_node, expected_raw_event_str, expected_len):
    ...
    def test_drive_order_of_context_messages(self, MockedAltAI):
        ...
        self.assert_message_node(result_msg_node,
                                 MessageNode(
                                     role=Role.assistant,
                                     message_content="Assistant Response",
                                 ))
    ...
    def test_driver_wrapper(self,  MockedAltAI):
        ...
        driver_wrapper_gen = self.create_driver_wrapper_instance(
            MockedAltAI, events, system_message, mock_event_formatter_fn, mock_function_call_spec_callable)
        ...
        self.assert_message_node(
            ...
            MessageNode(role=Role.system,
                        message_content="test_system_message_content")
        )

        self.assert_message_node(
            event_node.history[1],
            MessageNode(role=Role.user,
                        message_content="You need to parse test_event1")
        )

        self.assert_message_node(
            event_node.history[2],
            MessageNode(role=Role.assistant,
                        message_content="Assistant Response")
        )
    ...
    def test_driver_for_multiple_events(self, MockedAltAI):
        ...
        self.assert_message_node(
            ...
            MessageNode(role=Role.user,
                        message_content="You need to parse test_event2")
        )
    ...
    def test_interrogation(self, MockedAltAI):
        ...
        driver_wrapper_gen = self.create_driver_wrapper_instance(
            MockedAltAI,
            events, system_message, mock_event_formatter_fn,
