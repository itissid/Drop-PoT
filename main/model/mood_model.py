...
class MoodJsonTable(Base):  # type: ignore
    ...
    __table_args__ = (UniqueConstraint(
        'mood', 'flavor', 'version', name='mood_name_version'),)
...
class SubmoodBasedEmbeddingsTable(Base):  # type: ignore
    ...
    __table_args__ = (UniqueConstraint(
        'mood_id', 'sub_mood', 'embedding_text_composite_type', 'version', name='mood_submood_composite_version'),)


def insert_into_mood_json_table(
    mood: str, sub_moods: List[SubMood], flavor: MoodFlavors, version: str, engine: Engine
):
...
def get_mood_json_entries(mood: str, flavor: MoodFlavors, version: str, engine: Engine) -> List[MoodJsonTable]:
    Session = sessionmaker(bind=engine)
    session = Session()

    try:
        records = session.query(MoodJsonTable).filter(and_(
            MoodJsonTable.mood == mood,
            MoodJsonTable.flavor == flavor,
            MoodJsonTable.version == version)).all()
        return records
    finally:
        session.close()


def insert_accessor_entries(mood_id: int, submoods: List[SubMood], engine: Engine):
...
def generate_submoods_json_accessors(mood_id, submoods: List[SubMood]) -> List[SubmoodBasedEmbeddingTextAccessorTable]:
    """
    ...
    for idx, submood in enumerate(submoods):
        ...
        json_path = (
            f"{json_path_base}, {','.join(submood_place_act)}"
        )
        submood_entry = SubmoodBasedEmbeddingTextAccessorTable(
            submood_accessor_json_path=f"$[{idx}].SUB_MOOD",
...
def get_submood_embedding_text(mood: MoodJsonTable, version: str, json_path_records: List[SubmoodBasedEmbeddingTextAccessorTable]):
    ...
    for path_record in json_path_records:
        ...
        embedding_text_exprs = [
            parse(i.strip()) for i in path_record.embedding_accessor_json_path.split(",")]
        ...
        assert len(
            sub_mood_lst) == 1, f"Expected to find exactly one sub_mood for {path_record.submood_accessor_json_path} but found {len(sub_mood_lst)}"
        embedding_text_parts = []
        for embedding_text_expr in embedding_text_exprs:
            embedding_text_lst = [
                match.value for match in embedding_text_expr.find(sub_moods_json)]
            assert len(
                embedding_text_lst) == 1, f"Expected to find exactly one embedding_text for {path_record.embedding_accessor_json_path} but found {len(embedding_text_lst)}"
...
def insert_into_embeddings_table(engine: Engine, embedding_entries: List[dict]):
    ...
    try:
        for entry in embedding_entries:
            record = session.query(SubmoodBasedEmbeddingsTable).filter(
                ...
                SubmoodBasedEmbeddingsTable.embedding_text_composite_type == entry[
            ...
            if not record:
                embeddings_table_entry = SubmoodBasedEmbeddingsTable(
                    ...
                    embedding_text_composite_type=entry['embedding_text_composite_type'],
